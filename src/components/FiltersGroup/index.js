...
const FiltersGroup = props => {
  ...
  const renderTypeOfLocations = () => {
    const {employeeLocationList, onChangeEmployeeLocation} = props
    return (
      <div className="employment-type-container">
        <h1 className="employment-type-heading"> Location </h1>
        <ul className="employee-type-list-container">
          {employeeLocationList.map(eachEmployeeType => {
            const onSelectEmployeeLocation = event => {
              // console.log(event.target.checked)
              // console.log(event.target.value, event.target.checked)
              onChangeEmployeeLocation(event.target.value)
            }
            return (
              <li
                className="employee-item"
